...
def main():
    ...
    driver.get('https://www.qualtrics.com/support/survey-platform/managing-your-account/creating-account-logging/')
    sleep(7)

    # Find the nav element
    nav = driver.find_element(By.CSS_SELECTOR,'#main > div > div > nav > ul.flex-column.flex-nowrap.nav.support-menu')
    print("nav:",nav)
    ...
    for link in links_list:
        ...
        write_string_to_file(text,txt_output_file)
        # print("url:", url)
...
def get_text_from_url(url):
    ...
    pageContents = ''
    start = False
    pageTitle = soup.select_one("#main > div > div > article > header > div.align-items-baseline.d-flex.flex-wrap.justify-content-between.pb-3.row.no-gutters > h1")
